...
namespace TestCoin.Connections
{
    class MessageReader
    {
        ...
        private bool hasForked = false; //will only accept fork fix messages 
        private Connection forkCon; //only connection that node will communicate with when fork fixing
        private int workingIndex;



        public MessageReader(Form1 f, Blockchain chain, Func<bool> Pinger, ConnectionPool conPool, Tangle tangle = null)
        {
            this.form = f;
            this.chain = chain;
            this.tangle = tangle;
            this.tUO = new TangleUpdateObj(tangle);
            this.Pinger = Pinger;
        ...
        private void REQUEST(String content,out string response, out List<Packet> messages)
        {
            ...
            {
                String hash = Common.Common.splitAt(content, ".")[2];
                try
                {
                    TangleTransaction tran = tangle.reader.readTranThread(hash);

                    response = "RESPONSE.TRANSACTION.";
                    response += tran.getInfo();
                }
                catch(Exception e)
                {
            ...
            else if (content.Contains("GENESIS"))
            {
                response = "RESPONSE.GENESIS.";
                TangleTransaction tran = tangle.GetGenesis();
                response += tran.getInfo();
            }
            else
            {
        ...
        private void RESPONSE(String content,out string response, out List<Packet> messages)
        {
                    ...
                    try
                    {
                        ...
                        if (block.index == 0)
                        {
                            try
                            {
                                ...
                                if (Block.Compare(block, firstBlock))
                                {
                        ...
                        else
                        {
                            ...
                            if (block.index == lastBlock.index + 1)
                            {
                                ...
                                {
                                    chain.PendingTransRemoval(block);
            ...
            else if (content.Contains("TRANSACTION")) //response
            {
                ...
                {
                    response = "RESPONSE.TRANSACTION.ACK";
                    String hashContents = Common.Common.splitAt(content, ".TRANSACTION.")[1];
                    TangleTransaction tran = TangleTransaction.ParseTranText(hashContents);
                    Packet p;
                    String mes;

                    if (!tangle.DupeCheck(tran))
                    {
                        bool check1 = tangle.DupeCheck(tran.confirmAddress1);
                        bool check2 = tangle.DupeCheck(tran.confirmAddress2);
                        if (check1 && check2 && tUO.CheckFinished())
                        {
                            tangle.NewTransaction(tran);
                        }
                        else
                        {
                            if (!tUO.isInHash(tran.hash))
                            {
                                tUO.hashes.Insert(0,tran.hash);
                            }
                            if (!tUO.isInTrans(tran.hash))
                            {
                                tUO.trans.Add(tran);
                            }
                            if (!check1)
                            {
                                if (!tUO.isInHash(tran.confirmAddress1))
                                {
                                    tUO.hashes.Add(tran.confirmAddress1);
                                    p = new Packet(senderIP, senderPort, "REQUEST.TRANSACTION." + tran.confirmAddress1);
                                    messages.Add(p);
                                }
                            }
                            if (!check2)
                            {
                                if (!tUO.isInHash(tran.confirmAddress2))
                                {
                                    tUO.hashes.Add(tran.confirmAddress2);
                                    p = new Packet(senderIP, senderPort, "REQUEST.TRANSACTION." + tran.confirmAddress2);
                                    messages.Add(p);
                                }
                            }
                            if (tUO.CheckFinished())
                            {
                                AddTUOToTran();
                            }
                        }
            ...
            else if (content.Contains("GENESIS"))
            {
                ...
                {
                    String text = Common.Common.splitAt(content, "GENESIS.")[1];
                    TangleTransaction tran = TangleTransaction.ParseTranText(text);
                    TangleTransaction gen = tangle.GetGenesis();
                    if (!tran.hash.Equals(gen.hash))
                    {
                        //write gen to file reset cache
                        tangle.ResetTangle(tran);
                    }
                    response = "RESPONSE.GENESIS.ACK";
                }
            ...
            else if (content.Contains("TRANSACTION"))
            {
                ...
                else
                {
                    ...
                    if (!tangle.DupeCheck(tran))
                    {
                        ...
                        string message = "UPDATE.TRANSACTION." + tran.getInfo();
                        Connection sender = new Connection(senderIP, senderPort);
                        foreach (Connection con in conPool.pool)
                        {
                            if (!sender.Equals(con.IP, con.port))
        ...
        public void AddTUOToTran()
        {
            tUO.trans.Sort((x, y) => DateTime.Compare(x.preTimestamp, y.preTimestamp));

            List<TangleTransaction> list = new List<TangleTransaction>(tUO.trans); //copy for no modification errors :)

            foreach (TangleTransaction t in list)
            {
                tangle.NewTransaction(t,list);
            }
            tUO.Wipe();
        }
