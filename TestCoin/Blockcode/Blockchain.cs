...
namespace TestCoin.Blockcode
{
    public class Blockchain
    {
        ...
        Block lastBlock = null; //latest block on chain
        ...
        public void minePendingTrans(String rewardAddress, MiningTools.MiningSetup settings, out String blockDetails)
        {
            ...
            if (!isMining )
            {
                try
                {
                    ...
                    if (diff)
                    {
                        difficulty = calcDifficulty(lastBlock.index);
                    }
                    mineReward = Block.calculateReward(lastBlock.index + 1) + feeTotal;
                    ...
                    Block block = new Block(lastBlock.index + 1, chosenTrans, lastBlock.hash, mineReward, rewardAddress, feeTotal);

                    mineTime = Stopwatch.StartNew();
                    block.mineBlock(difficulty, this, block, settings.threadsUsed);
        ...
        public bool CreateUserTransaction(String publicAdd, String privKey, String receiveID, double amount, double fee, out String outputMessage)
        {
        ...
        public bool isChainValid(out List<Block> tempChain, Block block = null, List<Block> extraChain = null)
        {
            ...
            for (int i = 1; i < tempChain.Count; i++)
            {
                ...
                if (!HashTools.verifyMerkleRoot(tempChain[i].merkleRoot, tempChain[i])) { //error caughts?
                    return false;
                }
